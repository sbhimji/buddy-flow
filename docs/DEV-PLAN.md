...
- *Status:* closed on replayed 2026-08-11 flat files, all four mini-spec criteria passed. Code: `internal/ingest` (source-agnostic core: NBBO table, bounded-queue pipeline, capture-writer seam), `internal/feed` (file feeder; universe filter = subscription analog, `-full` stress mode), `internal/universe` (baskets-JSON loader), `cmd/ingest` (acceptance instrument; renamed `cmd/replay` 2026-08-14). Results: universe counts exact (25,920,623 T + 45,545,795 Q, matches independent Python measurement); **423k msg/sec sustained** end-to-end over the full-session replay vs the 120k floor, zero drops, max queue depth 8,016/2M; NBBO spot-checks (NVDA/SPY/AAPL @ 09:35:00) match an independent computation on every field incl. sequence; two runs bit-identical. **Scope amendment:** the live websocket feeder formally moved to 1.2 — capture-before-parse lives inside the socket read loop and no live run may precede capture, so the two are one story. Finding for 1.5: 719,206 trade-sequence regressions (2.8%) vs zero on quotes — the out-of-sequence/TRF-late population, exactly where the late-print metric expects it.
...
**1.2 — Live feeder & capture to disk** — mini-spec: `docs/mini-specs/1.2-live-feeder-capture.md` — **3 of 4 done-when criteria passed 2026-08-13**
- *Status:* live feeder + capture built, reviewed (10-finding review applied same day), and verified on real market data: smoke capture over the 08-13 close (954MB→1.0GB, 876k frames, 3.12M T + 3.40M Q, zero decode errors/unknown symbols); replay-twice bit-identical with counts matching live exactly (criterion 2 ✅); kill-drill passed — SIGKILL mid-stream detected via stale manifest, restart appended with exact resume provenance (`prior+this-run == total == file size`), torn line skipped, replay clean (criterion 3 ✅); criterion 1 (live session captured with manifest) ✅. **Open: criterion 4** — live counts vs vendor flat file reconciliation, blocked until Massive publishes the next session's file (~evening after). Live-wire discoveries for `data.md`: quotes carry `pt`; trades carry undocumented `ds`.
- ~~**Live websocket feeder** (moved here from 1.1, 2026-08-13): connect `wss://socket.massive.com/stocks`, subscribe `T.`/`Q.` for the 189 universe symbols, decode into the `internal/ingest` structs (ms→ns timestamp scaling; `pt` field absent from the vendor Go client — parse raw JSON, confirm on the wire). Capture-before-parse lives inside the socket read loop, which is why feeder and capture are one story.~~ Done (`internal/feed/live.go`, `internal/capture`, `cmd/capture` — renamed `cmd/live` 2026-08-14); fatal-stop on auth failure/unwritable capture, reconnect with backoff-reset + gap control records.
- ~~Every raw message (both feeds) appended to per-session capture files with receive timestamps. Capture is unconditional from the first live run — recorded sessions are the project's test corpus.~~ Done — raw frames (not decoded structs), `<recv_ns> <frame>` lines, no off-switch.
...
- *Done when:* a session capture replays through the ingest path and produces identical message counts ~~and identical bucket outputs~~ twice [§5 Days 1–3 "Done when"]. *(Amended at mini-spec: bucket outputs belong to 1.4 — buckets don't exist yet. Counts+NBBO verified ✅; the story closes on criterion 4 + first full-session capture.)*

**~~1.3 — Replay harness~~** ✅ resolved 2026-08-13 — mini-spec: `docs/mini-specs/1.3-replay-harness.md`
- *Status:* closed same day. `-speed` on `cmd/ingest -capture` (now `cmd/replay -capture`): 0=instant, 1=real-time (absolute-schedule pacing by capture recv_ns; measured exact to ~1ms on a 3s slice), N=accelerated; 30s cap on dead gaps. Content identical at every speed (verified instant vs ×100 on the full 08-13 capture). The shared flat-file reader extraction moved to `docs/backlog.md` (no second CSV consumer yet; recorded, not dropped).
...
**~~1.4 — Bucket store [F7]~~** ✅ resolved 2026-08-14 — mini-spec: `docs/mini-specs/1.4-bucket-store.md`
- *Status:* closed on the replayed 2026-08-13 capture. Code: `internal/bucket` (RAM-resident store keyed by SIP epoch-second; first pipeline consumer of the 0.3 classifier — per-class splits + unknown-condition tripwire; sorted self-describing CSV, atomic write, gzip-readable; reader with zero-buckets for silent seconds and `DeriveMinute` summing sixty 1-second buckets), `Observer` seam in `internal/ingest` (bucket, later Phase 3 consumers, plug into the pipeline without an import cycle). Wired: `cmd/replay -buckets` (whole-session runs only, validated at flag parse) and `cmd/live` (single write at session close; mid-session snapshots removed per mini-spec amendment — 3.0 owns live visibility). Results: sampled rows — all 8,920 (sym,sec) buckets for six symbols — match an independent Python recomputation from raw `stream.jsonl` on every field exactly; two replays byte-identical (required forcing rounding of `price*size` — Go's arm64 FMA fusion would otherwise make bucket files differ across CPU architectures); minute = Σ seconds by reader helper + test. First live close-write exercises Monday 2026-08-17 (the 08-14 session's capture predates the wiring; its buckets regenerate by replay — the D2 principle). *(Commands renamed this story: `cmd/ingest`→`cmd/replay`, `cmd/capture`→`cmd/live`.)*
- ~~Store at 1-second resolution for the entire session, all day (cheap at 60 tickers); 1-minute and rolling views are *derived*, never a second storage format. This removes the 10:00 resolution seam that would otherwise distort CUSUM and slopes.~~ Done — one resolution, coarser views derived at read time only.
- Persist every computed metric per bucket as it comes to exist in later phases (design principle: **store everything, model nothing** — this is the future-ML dataset, the replay scrubber's source, and the grading ledger's input, all for free). *(Standing principle for Phases 3+ — the CSV is self-describing, and the column-compatibility contract binding future column-adders is recorded in `docs/backlog.md`.)*
- ~~Zero-trade buckets: FlowShare 0/positive = 0; define 0/0 (whole universe silent) as carry-forward-blank and render as gap [F13 addendum].~~ Reader returns zero-buckets for absent rows (0/positive = 0); the 0/0 carry-forward-blank belongs to the metric layer, which can see the universe (mini-spec D5).
- *Done when:* ~~buckets from a replayed session match a hand-computed sample and derived 1-minute bars equal the sum of their 1-second bars~~ — both passed 2026-08-14 (see Status).

*(1.5 ingest data-quality monitoring moved to `docs/backlog.md` 2026-08-15 — priority is trader-visible output (2.1 → 3.0), and 1.5's formal surfaces don't exist yet (dev view = 3.0, ledger = 6.5), so it re-enters with those stories. The protective signals already run in code and stay loud: sequence-regression / decode-error / unknown-symbol / cond-overflow counters, the unknown-condition tripwire, manifest counts, capture-integrity-by-manifest-absence. The lateness study feeding 3.3 was already a separate backlog entry; 1.2's capture-vs-flat-file reconciliation is unaffected and stays on the critical path. Full metric list and re-entry criteria in the backlog entry.)*
...
- Calendar policy [F11]: ~~exclude half days; flag-and-configurably-exclude FOMC/CPI-class event days (Decision D6)~~ **calendar moved to backlog (TOP item) 2026-08-15** — Monday-POC push; numerically safe now because D4 is flag-and-*include* (flags change no number) and no half-day enters a late-August lookback (must land before Thanksgiving week). v1 interim: trading days inferred from aggs responses, every day treated full. Kept in-story: a ticker contributes to z-scores only after ≥10 profiled days.
...
- **Scope amendment (2026-08-14):** v1 classifies **on-time prints only** — `sip_timestamp − participant_timestamp` within a small tolerance — against the current in-memory NBBO; late prints are unclassified (DeltaRatio denominator only). The ring-buffer window for classifying late prints against historical NBBO is deferred until the flat-file lateness study sizes it from measured percentiles (`docs/backlog.md`, "Late-print lateness analysis").
